...
**Task 5: Volume open and ops vtable** ✅ DONE (3bbc1eb, d9bc3af)
...
**Task 6: BDL read / write / erase** ✅ DONE (c8faf1b, aeb6e36)
...
- Test cases:
  ...
  Items 1-5 landed incidentally as part of Tasks 4/5/6 (a0d16e3, d9bc3af,
  aeb6e36 respectively) — each feature's own test commit already exercised its
  Task 7 scenario using `spi_nand_flash`'s Linux mmap emulation
  (`nand_linux_mmap_emul.h`) rather than `esp_blockdev_util/memory`: it models
  real NAND semantics (page/block program constraints, bad-block ioctls) that
  a generic memory BDL doesn't, so it is a more faithful fake NAND for this
  component's needs. Test files ended up split by concern rather than living
  in one `nand_ubi_test.c`: `test_nand_ubi.cpp` (media/CRC/EBA units),
  `test_nand_ubi_attach.cpp`, `test_nand_ubi_volume.cpp`, `test_nand_ubi_rw.cpp`.
  Item 6 landed in `test_nand_ubi_leak.cpp` plus AddressSanitizer wiring in
  `host_test/main/CMakeLists.txt` (applied to both the test binary and the
  `esp_nand_ubi` component lib, matching ESP-IDF's own `tcp_transport`/`mqtt`
  host_test apps — no repo-wide Kconfig convention for this exists). Full
  suite: 57 cases / 1194 assertions, all passing under ASAN. Note:
  LeakSanitizer's exit-time check only runs on Linux (Apple Clang's ASAN on
  macOS has no LSan), so a genuine leak verdict needs a Linux run — matches
  CI's `run-target-linux` job rather than a local macOS build.
