...
esp_err_t nand_wrap_prog(spi_nand_flash_device_t *handle, uint32_t page, const uint8_t *data, uint32_t oob_lpn)
{
    esp_err_t ret = ESP_OK;
    xSemaphoreTake(handle->mutex, portMAX_DELAY);
...
esp_err_t nand_wrap_copy(spi_nand_flash_device_t *handle, uint32_t src, uint32_t dst, uint32_t oob_lpn)
{
    esp_err_t ret = ESP_OK;
    xSemaphoreTake(handle->mutex, portMAX_DELAY);
