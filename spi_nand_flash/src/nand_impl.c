...
{
    nand_ecc_status_t bits_corrected_status = NAND_ECC_MAX;

    if (dev->decode_ecc_status) {
        bits_corrected_status = dev->decode_ecc_status(dev, status);
    } else {
        ESP_LOGE(TAG, "%s: ECC decoder not installed", __func__);
    }
