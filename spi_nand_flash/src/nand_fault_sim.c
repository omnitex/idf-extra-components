...
esp_err_t nand_prog(spi_nand_flash_device_t *handle, uint32_t page, const uint8_t *data, uint32_t oob_lpn)
{
    ...
    if (oob_lpn != DHARA_OOB_LPN_NONE) {
        ...
        ESP_RETURN_ON_ERROR(
            nand_emul_write(handle, data_offset + handle->chip.page_size + 4,
                            lpn_buf, sizeof(lpn_buf)),
            TAG, "nand_prog: write LPN");
    }
    return ESP_OK;
}
...
esp_err_t nand_copy(spi_nand_flash_device_t *handle, uint32_t src, uint32_t dst, uint32_t oob_lpn)
{
    ...
    if (oob_lpn != DHARA_OOB_LPN_NONE) {
        ...
        ESP_RETURN_ON_ERROR(
            nand_emul_write(handle, dst_offset + handle->chip.page_size + 4,
                            lpn_buf, sizeof(lpn_buf)),
            TAG, "nand_copy: write LPN");
    }
    return ESP_OK;
}
...
{
    uint8_t lpn_buf[4];
    uint32_t oob_lpn_offset = page * handle->chip.emulated_page_size + handle->chip.page_size + 4;

    ESP_RETURN_ON_ERROR(
        nand_emul_read(handle, oob_lpn_offset, lpn_buf, sizeof(lpn_buf)),
        TAG, "nand_read_lpn");
