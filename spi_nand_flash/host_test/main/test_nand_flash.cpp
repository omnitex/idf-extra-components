...
TEST_CASE("verify nand_prog, nand_read, nand_copy, nand_is_free works", "[spi_nand_flash]")
{
    ...
    REQUIRE(nand_wrap_prog(device_handle, test_page, pattern_buf, DHARA_OOB_LPN_NONE) == 0);
    ...
    REQUIRE(nand_wrap_copy(device_handle, test_page, dst_page, DHARA_OOB_LPN_NONE) == 0);
