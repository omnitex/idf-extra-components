...
The system SHALL support optional byte-level tracking of individual write operations when enabled in configuration.

#### Scenario: Byte-level tracking disabled
- **WHEN** advanced emulator is initialized with `track_byte_level = false`
- **THEN** backend's `on_byte_write()` SHALL NOT be called during write operations

#### Scenario: Byte-level tracking enabled
- **WHEN** advanced emulator is initialized with `track_byte_level = true`
- **THEN** backend SHALL receive `on_byte_write()` callback for each byte modified during writes
- **AND** SHALL track per-byte write count and last write timestamp

#### Scenario: Backend does not support byte tracking
- **WHEN** backend's `on_byte_write()` returns `ESP_ERR_NOT_SUPPORTED`
...
The default sparse hash backend SHALL only allocate metadata for blocks/pages/bytes that have been written or erased.
...
- **WHEN** workload writes to 1% of total flash capacity
- **THEN** metadata memory usage SHALL be less than 1% of what a dense array would require

### Requirement: Aggregate statistics
The system SHALL compute aggregate wear statistics across all blocks including min/max/average erase counts.

#### Scenario: Calculate wear leveling efficiency
- **WHEN** developer calls backend's `get_stats()` after mixed operations
- **THEN** backend SHALL return `wear_leveling_efficiency` calculated as `(max_block_erases - min_block_erases) / avg_block_erases`
